...
impl<'a> Render for RenderState<'a> {
    fn render(&mut self, shape: &Shape, transformation: SpriteTransform) -> () {
        ...
        let matrix = transformation
            .position
            .post_transform(&RenderState::viewport_transform(self.viewport))
            .to_row_arrays();
        ...
        self.target
            .draw(&*vbo, &ebo, &self.program, &uniforms, &RenderState::draw_parameters())
            .unwrap();
    }
}
...
impl<'a> RenderState<'a> {
    ...
    fn viewport_transform(viewport: (u32, u32)) -> Transform2D<f32, (), ()> {
        Transform2D::create_scale(
            RenderState::BASE_SCALE / viewport.0 as f32,
            -RenderState::BASE_SCALE / viewport.1 as f32,
        )
    }
}
